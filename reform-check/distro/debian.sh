...
backend_boot_tool_checks() {
  ...
  elif ! printf "$flashkerneldefaultcontent" | cmp --quiet - /etc/default/flash-kernel; then
...
backend_repo_checks() {
  ...
  elif ! printf "$aptprefcontent" | cmp --quiet - /etc/apt/preferences.d/reform.pref; then
